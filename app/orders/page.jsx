...
export default function OrderSystem() {
    ...
    // Always poll — history is visible on all layouts
    useEffect(() => {
        fetchHistory();
    ...
    const getCorndogOrderActions = useCallback(
        ({ orderNumber, items }) => {
            ...
            const orderPhone = history.flat().find((item) => item.orderNumber === orderNumber)?.phone;
            ...
            if (!orderPhone) {
                return <span className='text-xs font-semibold text-green-700'>All Ready ✓</span>;
            }
    ...
    return (
        <div className='h-full'>
            {/* ── iPhone portrait: Order + Cart only ── */}
            ...
            {/* ── iPad landscape: 3-column full-height ── */}
            <div className='hidden lg:grid lg:grid-cols-3 h-full divide-x divide-gray-200'>
                {/* Col 1: Order + Cart */}
