...
test('a pack-only category is named after itself, with the pack as a modifier', () => {
    const cookie = categoryFor('Cookie');
    const line = buildCartLine(cookie, select(cookie, { pack: '1 for $5' }));

    assert.equal(line.name, 'Cookie');
    assert.deepEqual(line.modifiers, ['1 for $5']);
    assert.equal(line.price, 5.0);
    assert.equal(formatItemName(line), 'Cookie (1 for $5)');
});

test('each cookie pack charges its own price, not a per-cookie multiple', () => {
    const cookie = categoryFor('Cookie');
    const priceOf = (pack) => buildCartLine(cookie, select(cookie, { pack })).price;

    assert.equal(priceOf('1 for $5'), 5.0);
    assert.equal(priceOf('3 for $14'), 14.0);
    assert.equal(priceOf('5 for $23'), 23.0);
});

test('a cookie cannot be added until a pack is chosen', () => {
    const cookie = categoryFor('Cookie');

    assert.equal(isSelectionComplete(cookie, emptySelection(cookie)), false);
    assert.equal(buildCartLine(cookie, emptySelection(cookie)), null);
});
...
test('the panel shows a range when the options carry the price', () => {
    assert.equal(priceLabelFor(categoryFor('Side')), '$1.00 – $5.00');
    assert.equal(priceLabelFor(categoryFor('Cookie')), '$5.00 – $23.00');
