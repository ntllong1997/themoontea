...
export const CATEGORIES = [
    ...
    // choice alone would collide with a same-named item in another category,
    // and the sales summary groups by name. See the uniqueness test in
    // catalog.test.js.
    //
    // Every cookie is the same, so there is no flavour to pick: the only choice
    // is how many, and the pack carries the price rather than the category.
    {
        key: 'Cookie',
        label: 'Cookie',
        orderable: true,
    ...
    // Like Cookie, the price lives on the options rather than on the category:
    // a water is $1, a soda $2 and a flan $5. `priceLabelFor` shows the range.
