...
/**
 * A local wall-clock timestamp string, e.g. `2026-07-10T00:00:00`.
 *
 * The `timestamp` column is `timestamp without time zone` holding the till's
 * local wall clock, so bounds have to be sent the same way. `toISOString()`
 * would shift them by the UTC offset — five hours in US Central — which is
 * exactly what makes a late-night order vanish from its own day.
 */
...
export async function getOrdersInRange(from = null, to = null) {
    ...
    if (from) query = query.gte('timestamp', toLocalTimestamp(from));
    if (to) query = query.lt('timestamp', toLocalTimestamp(to));
