...
import { buildPrintJob } from '@/lib/orders/printQueue';
...
const PRINT_JOBS_TABLE = 'print_jobs';
...
export async function createOrder({
    ...
    await enqueuePrintJob({ orderNumber, timestamp });
...
/**
 * Queues the order for the receipt-printer bridge.
 *
 * Deliberately never throws: the order is already committed by this point, and
 * failing the checkout because the *printer queue* was unreachable would be a
 * worse outcome than a receipt that prints late. The bridge's safety poll
 * picks up anything missed here.
 *
 * `ignoreDuplicates` leans on the table's unique (order_number,
 * order_timestamp) constraint, so a retry can never enqueue the same receipt
 * twice.
 *
 * @param {{ orderNumber: number, timestamp: string }} params
 */
async function enqueuePrintJob({ orderNumber, timestamp }) {
    try {
        const { error } = await supabase
            .from(PRINT_JOBS_TABLE)
            .upsert(buildPrintJob({ orderNumber, timestamp }), {
                onConflict: 'order_number,order_timestamp',
                ignoreDuplicates: true,
            });

        if (error) {
            console.error(`Could not queue order ${orderNumber} for printing:`, error);
        }
    } catch (err) {
        console.error(`Could not queue order ${orderNumber} for printing:`, err);
    }
}
