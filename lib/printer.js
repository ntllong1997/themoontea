...
export async function printReceipt({ orderNumber, items, taxRate, paymentUrl = null }) {
    const res = await fetch(`${PRINT_SERVER}/print`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ orderNumber, items, taxRate, cashappUrl: getActiveCashApp(), paymentUrl }),
    });
    if (!res.ok) {
