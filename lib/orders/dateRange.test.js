...
// These assert instants rather than wall-clock text, so they hold in any
// machine time zone.

test('a stored timestamp with no zone marker is read as UTC, not local', () => {
    // Both tills write UTC into a `timestamp without time zone` column, so the
    // value comes back bare. Reading it as local time would shift an evening's
    // takings onto the next day in every US time zone.
    assert.equal(
        parseOrderTimestamp('2026-07-27T01:57:50.441').getTime(),
        Date.UTC(2026, 6, 27, 1, 57, 50, 441)
    );
});

test('the space-separated Postgres spelling is read the same way', () => {
    assert.equal(
        parseOrderTimestamp('2026-07-27 01:57:50').getTime(),
        Date.UTC(2026, 6, 27, 1, 57, 50)
    );
});

test('an explicit zone on the value is respected rather than overridden', () => {
    assert.equal(
        parseOrderTimestamp('2026-07-27T01:57:50.441Z').getTime(),
        Date.UTC(2026, 6, 27, 1, 57, 50, 441)
    );
});

test('a late evening order counts under the day it was actually sold', () => {
    // 2026-07-27T01:00Z is 8pm on the 26th in US Central. A window covering
    // the 26th in that zone must keep it, and the 27th's must not.
    const soldAt = '2026-07-27T01:00:00.000';
    const twentySixth = { from: new Date(Date.UTC(2026, 6, 26, 5)), to: new Date(Date.UTC(2026, 6, 27, 5)) };
    const twentySeventh = { from: new Date(Date.UTC(2026, 6, 27, 5)), to: new Date(Date.UTC(2026, 6, 28, 5)) };

    assert.ok(isWithinWindow(soldAt, twentySixth), 'belongs to the trading day it was sold on');
    assert.ok(!isWithinWindow(soldAt, twentySeventh), 'must not slide onto the next day');
});
